...
function adjustTabHeights() {
    ...
    document.querySelectorAll('.tab-content').forEach(tabContent => {
        ...
        // Find the tallest tab pane
        tabContent.querySelectorAll('.tab-pane').forEach(pane => {
            pane.style.display = 'block'; // Temporarily show to measure
            maxHeight = Math.max(maxHeight, pane.scrollHeight);
            pane.style.display = ''; // Reset
        });
        
        // Set the height for all panes in this tab group
        tabContent.style.minHeight = maxHeight + 'px';
    });
}

// Run on page load and when tabs are clicked
window.addEventListener('load', adjustTabHeights);
document.querySelectorAll('[data-bs-toggle="tab"], [data-toggle="tab"]').forEach(tab => {
    tab.addEventListener('click', adjustTabHeights);
});